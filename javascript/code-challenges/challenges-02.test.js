...
const raisedToTheThird = (arr) => {   //take in an array of numbers
  // Solution code here...
  let tempArrary = [];                //create a work space
  arr.forEach(value => {              //for each index in the array a value is pulled
    let temp = Math.pow(value, 3);    //temporary storage is created and is filled with the value from the array^3
    tempArrary.push(temp);            //the new value is pushed into the new array
  });
  return tempArrary;                  //tempArrary must be returned to raisedToTheThird.
};
