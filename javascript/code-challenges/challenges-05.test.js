...
const toLastNames = people => {
  // Solution code here...+
  //WORKED BUT DIDN'T USE REDUCE
  // USED CHALLENGE 2:2 AS A TEMPLATE
  return people.map(people => (people.firstName +' '+ people.lastName)); 

  
  //NOPE
  // let fullName = (people) => fullName.map(people.reduce((a, b) => {
  //   (a.firstName + ' ' + b.lastName);
    // let fullName = [];
    // let fullName = new Map();
    // people.map(value.firstName + ' ' + value.lastName);
...
// Array.reduce()
// -runs a callback (receiving accumulator, value, index) on every element
// -returns a new thing based on the accumulator
// -does NOT modify the original array
// -array.reduce((accumulator, value, index) => initialValue)
// -can be used to shape data received from an API
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const addValues = (arr) => {
  // Solution code here...
  const tempArrary = arr.reduce( (accumulator, val) => {
    return accumulator + val;
  }, 0);  //INITIAL ACCUMULATOR VALUE IS REQUIRED!!!
  return tempArrary;
  //ALL OF MY ATTEMPTS TO REFACTOR RETURNED 'UNDEFINED'.
...
const addPurchases = (arr) => {
  // Solution code here...
  let tempArrary = arr.reduce((accumulator, val) => {
    return accumulator + val.purchasePrice;  //WHY IS RETURN REQUIRED?
  }, 0);
  return tempArrary;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let tempArrary = arr.reduce((accumulator) => {
    return accumulator + 1;  //WHY IS RETURN REQUIRED?
  }, 0);
  return tempArrary;
};
...
const returnNames = (arr) => {
  // Solution code here...
  let tempArrary =[];
  arr.reduce((accumulator, object) => {  //ACCUMULATOR IS NOT USED
    tempArrary.push(object.name);
  }, []);
  return tempArrary;
};
...
const reversedString = (str) => {
  // Solution code here...
  //INSPIRED/COPIED FROM JOHN'S REDUCE VIDEO
  let arr = str.split('');
