...
const getNames = (arr) => {
  // Solution code here...
  //WITH HELP FROM SHEYLA
  let tempArrary = [];
  arr.forEach(arr => {              //forEach pulled the arrays in one at a time
    let temp = arr.name.split('');  //arr.name is the name in the object
                                    //splits the name string into an array of characters
    // console.log(`temp = ${temp}.`); //should reference split array on string arr.name provided
    // console.log(`arr.name = ${arr.name}.`);//name from the object
    // console.log(`arr.name.split = ${arr.name.split('')}.`);  //splits the name string into an array of characters
    let reversed = temp.reduce((newString, currentLetter) => {
    return currentLetter + newString;
  })
    tempArrary.push(reversed);
  }) 
  return tempArrary;
};
  ...
  // let temp = (reverse(arr[i].name));
  // tempArrary.push(temp);

  // return arr.map(arr => (arr[i].name);
  // return arr.map(arr =>)
  // CHALLENGE 05 USE OF MAP
  // return people.map(people => (people.firstName +' '+ people.lastName));

  //CHALLENGE 05 USE OF REDUCE TO REVERSE.  reverse() was not allowed.
  // let arr = str.split('');
